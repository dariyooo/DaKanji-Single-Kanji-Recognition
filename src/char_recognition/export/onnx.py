...
def export_onnx(
    ...
    *,
    ...
    Args:
        model: a recognizer mapping a raw image ``(B, C, H, W)`` to logits.
        path: output ``.onnx`` path.
        image_size: example size used for tracing.
        opset: ONNX opset version.
        dynamic: if True, mark batch/height/width as dynamic.
        probabilities: wrap with softmax so the artifact outputs class probabilities.
    """
    path = Path(path)
    ...
    dynamic_shapes = {"x": {0: Dim("batch"), 2: Dim("height"), 3: Dim("width")}} if dynamic else None
...
def onnx_parity(
    ...
    *,
    ...
    """Max ``|ONNX - PyTorch|`` over a few input sizes; needs onnxruntime.

    Compares the exported graph (softmax probabilities) against the model, including the
    in-graph resize, at sizes other than the one traced. Returns ``(h, w, max_abs_diff)``.
    """
    import numpy as np
    ...
    for height, width in sizes or [image_size, (128, 96), (220, 300)]:
        sample = np.random.randint(0, 256, (1, in_channels, height, width)).astype("float32")
        onnx_out = session.run(None, {input_name: sample})[0]
        with torch.no_grad():
            torch_out = reference(torch.from_numpy(sample)).numpy()
        report.append((height, width, float(np.abs(onnx_out - torch_out).max())))
    return report
